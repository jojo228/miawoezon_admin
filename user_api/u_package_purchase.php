...
header('Content-Type: application/json'); // Use application/json for proper JSON responses
...
if (!isset($data['uid']) || !isset($data['plan_id']) || !isset($data['transaction_id']) || !isset($data['pname'])) {
    $returnArr = array(
        "ResponseCode" => "400", // Use 400 for Bad Request
        "Result" => "false",
    ...
    // Validate that uid and plan_id are positive integers
    if ($uid <= 0 || $plan_id <= 0) {
        $returnArr = array(
            "ResponseCode" => "400",
            "Result" => "false",
        ...
        if (!$fetch) {
            ...
            $field_values = array(
                "uid", "plan_id", "p_name", "t_date", "amount",
                "day", "plan_title", "plan_description", "expire_date",
                "start_date", "trans_id", "plan_image"
            );
            $data_values = array(
                $uid, $plan_id, $pname, $datetime, $fetch['price'],
                $fetch['day'], $fetch['title'], $fetch['description'],
                $till_date, $current_date, $transaction_id, $fetch['image']
            );
            $h = new Estate();
            $check = $h->restateinsertdata_Api($field_values, $data_values, $table);
